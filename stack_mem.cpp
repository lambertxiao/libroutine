...
StackMem::StackMem(int stack_size) : stack_size_(stack_size), occupy_rt_(nullptr), stack_buffer_((char*)malloc(stack_size)) {

  // 栈由高地址往低地址增长
  stack_bp_ = stack_buffer_ + stack_size;
}

ShareStack::ShareStack(int count, uint64_t stack_size) : count_(count), stack_size_(stack_size) {
